...
class WatchdogService:
        ...
        with cls._lock:
            if cls._instance is None:
                if repository is None or notifier is None or config is None:
                    raise ValueError("Repository, notifier, and config must be provided when creating instance")
        ...
        try:
            ...
            for alert in alerts:
                if "labels" in alert and "alertname" in alert["labels"]:
                    ...
                    if alertname == self.config.expected_alertname and status == "firing":
                        logger.info(f"Valid Watchdog alert received: {alertname} (status: {status})")
                        return alert
    ...
    def get_health_status(self):
        ...
        return {
            "status": status,
            "last_watchdog_received": WatchdogState.format_timestamp(last_watchdog_time),
