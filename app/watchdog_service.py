...
log_level = getattr(logging, log_level_name, logging.DEBUG)  # Fallback auf DEBUG wenn ungültig
    ...
    if os.path.exists(PERSISTENCE_FILE):
        try:
            ...
            with watchdog_state_lock:
                ...
                watchdog_state["last_status_notification"] = time.time()  # Auch den Status-Timestamp setzen
    else:
        with watchdog_state_lock:
...
def save_watchdog_state():
    ...
    try:
        with watchdog_state_lock:
            state_copy = watchdog_state.copy()  # Kopie erstellen für thread-sichere Speicherung
